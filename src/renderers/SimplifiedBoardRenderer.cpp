...
void SimplifiedBoardRenderer::PrintWhiteBoard() const
{
  ...
  for (int row = 8; row > 0; row--)
  {
    ...
    if (row == 8)
      cout << "   BLACK CAPTURED PIECES: " << board.GetCapturedPieces(PieceColor::BLACK, true);
    if (row == 7)
      cout << "   WHITE CAPTURED PIECES: " << board.GetCapturedPieces(PieceColor::WHITE, true);
    if (row != 1)
      cout << "\n   " << middle << "\n";
    else
      cout << "\n   " << bottom << "\n";
...
std::string_view SimplifiedBoardRenderer::PieceToString(const std::shared_ptr<Piece> piece) const
{
